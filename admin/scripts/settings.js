        ...
        function get_contacts() {
            ...
            //no iframe bcz address,gmap can be targeted by document.getElementById but not iframe
            ...
            xhr.onload = function() {
                contacts_data = JSON.parse(this.responseText); //data coming from settings_crud in json format
                // console.log(contacts_data);
                contacts_data = Object.values(contacts_data);


                for (i = 0; i < contacts_p_id.length; i++) {
                    document.getElementById(contacts_p_id[i]).innerText = contacts_data[i +
                        1]; //i+1 because there is sr_no in index 0 so we are targetting index 1 which is address.
                }
        ...
        function contacts_inp(data) {
            ...
            for (let i = 0; i < contacts_inp_id.length; i++) {
                document.getElementById(contacts_inp_id[i]).value = data[i + 1]; // i+1 because sr_no is in db table at index 0
            }
        };
        ...
        function add_member() {
            ...
            data.append('picture', member_picture_inp.files[0]); //only one file 
        ...
        function get_members() {
            ...
            xhr.send("get_members"); //this will hits the settings_crud.php

        }


        function rem_member(val) { //val="sr_no" from settings_crud's html.
            // console.log(val);
            let xhr = new XMLHttpRequest();
            xhr.open("POST", "ajax/settings_crud.php", true);
            xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
