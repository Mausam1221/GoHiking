<?php
    ...
    if(isset($_POST['get-all-rooms']))
    {
        ...
        while($row=mysqli_fetch_assoc($res))
        {
            ...
            if($row['status']==1)
            {
                $status="<button onclick='toggle_status($row[id],0)' class='btn btn-dark btn-sm rounded shadow-none'>Active</button>";

            }
            else
            {
            $status = "<button onclick='toggle_status($row[id],1)' class='btn btn-warning btn-sm rounded shadow-none'>Inactive</button>";
            ...
            <td>$status</td>
            <td>
                <button type='button' onclick='edit_details($row[id])' class='btn btn-dark btn-sm shadow-none' data-bs-toggle='modal' data-bs-target='#edit-room'><i class='bi bi-pencil-square'></i>
                </button>
...
if (isset($_POST['get_room_images'])) {
    ...
    while($row=mysqli_fetch_assoc($res))
    {
        ...
        if($row['thumb']==1)
        {
            $thumb_btn="<i class='bi bi-check-lg bg-success text-light px-2 py-1 rounded fs-5'></i>";
        }
        else
        {
